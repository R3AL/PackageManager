    ...
    m_valid(true)
{
    ...
    for( const auto& sec : scriptSections )
    {
        ...
        auto section = Switch(sectionName)	.Case("compilers",		Section::Compilers)
        ...
        if( section == Section::Compilers )
        {
            ...
            for( const auto& element : contentElements )
            {
                ...
                auto compiler = Switch(key)	.Case("MSVC",	Compiler::MSVC)
                        ...
                        const std::vector<std::string>& arguments) const -> void
{
    ...
    auto scriptCommand = Switch(command).Case("print",	ScriptCommand::Print)
